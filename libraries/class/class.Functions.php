...
class Functions
{
	...
	public function showProduct($v, $col='')
	{
		...
		$noimage="this.src='".THUMBS."/280x230x2/assets/images/noimage.png'";
		$rt='';
		$rt.='<div class="'.$col.'">';
		$rt.='<div class="pro">';
		$rt.='<div class="img_pro img_"><a href="'.$v[$sluglang].'" title="'.$v['ten'.$lang].'" >';
		$rt.='<img class="img-responsive" alt="'.$v['ten'.$lang].'"  onerror='.$noimage.' src="'.THUMBS.'/280x230x1/'.UPLOAD_PRODUCT_L.$v['photo'].'" > ';
		$rt.='</a></div>';
		$rt.='<h3 class="name_pro"><a href="'.$v[$sluglang].'" title="'.$v['ten'.$lang].'" >'.$v['ten'.$lang].'</a></h3>';
		$rt.= '<div class="price_pro">'. $this->get_gia_pro($v['gia'],$v['giamoi']) .'</div>';
		$rt.='</div>';
		$rt.='</div>';
		echo $rt;
	}
		...
		if(($gia>0) & ($giamoi>0)){
			...
			if($gia>0){
				...
				if($giamoi>0){
					...
					$rt_price.='<a href="lien-he" title="'.lienhe.'">'.lienhe.'</a>';
				}
			}
		}
		return $rt_price;
	...
	public function show_newss($v, $col='')
	{
		global $lang,$sluglang;
		$noimage="this.src='".THUMBS."/380x250x2/assets/images/noimage.png'";
		$rt_newss='';
		$rt_newss.='<div class="'.$col.'">';
		$rt_newss.='<div class="newss_box">';
		$rt_newss.='<div class="newss_img img_"><a href="'.$v[$sluglang].'" title="'.$v['ten'.$lang].'" >';
		$rt_newss.='<img class="img-responsive" alt="'.$v['ten'.$lang].'"  onerror='.$noimage.' src="'.THUMBS.'/380x250x1/'.UPLOAD_NEWS_L.$v['photo'].'" > ';
		$rt_newss.='</a></div>';
		$rt_newss.='<div class="newss_content flex_row">';
		$rt_newss.='<div class="newss_date"><span>'.date("d",$v['ngaytao']).'</span><p>'.date("m/Y",$v['ngaytao']).'</p></div>';
		$rt_newss.='<h3 class="newss_name"><a href="'.$v[$sluglang].'" title="'.$v['ten'.$lang].'" >'.$v['ten'.$lang].'</a></h3>';
		$rt_newss.='</div>';
		$rt_newss.='<div class="newss_describe"><p>'.$v['mota'.$lang].'</p></div>';
		$rt_newss.='</div>';
		$rt_newss.='</div>';
		echo $rt_newss;
	}
	...
	public function showNews($v, $col='')
	{
		...
		$rt='';
		$rt.='<div class="'.$col.'">';
		$rt.='<div class="box_news flex_row">';
		$rt.='<div class="img_news img_"><a href="'.$v[$sluglang].'" title="'.$v['ten'.$lang].'" >';
	...
	public function check_permission()
	{
		...
		if($_SESSION[$login_admin]['role'] == 3 || !empty($config['website']['debug-developer'] == true)) return false;
		else return true;
	}
